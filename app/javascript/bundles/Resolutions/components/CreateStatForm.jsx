...
const CreateStatForm = ({goals, refresh, createStat}) => {
  const { handleChange , register, handleSubmit, reset, formState: {errors}} = useForm();
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)}>
      <div className="flex flex-col mt-2 text-xl">
        ...
        <div className="col-span-full mt-2">
          <label htmlFor="title" className="block text-sm font-medium leading-6 text-gray-900">I took</label>
        </div>
        ...
        <div className={"mt-2"}>
          <DatePicker selected={activityAt} onChange={(date) => setActivityAt(date)} />
        </div>
      </div>
